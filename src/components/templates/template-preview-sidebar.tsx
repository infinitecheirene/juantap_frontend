...
export function TemplatePreviewSidebar({ template }: TemplatePreviewSidebarProps) {
  ...
  // Backend statuses
  const [savedStatus, setSavedStatus] = useState<"saved" | "bought" | "pending" | "free" | null>(template.category === "premium" ? null : "free")
  ...
  useEffect(() => {
    ...
    Promise.all([fetchSavedTemplates(), fetchBoughtedTemplates(), fetchUsedTemplates()]).finally(() => setIsLoadingStatus(false))
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="text-center">
              {isPremium ? (
                <>
                  <span className="text-3xl font-bold text-gray-900">{formatPrice(template.price as number)}</span>
                  {template.original_price && <span className="ml-2 text-xl text-gray-500 line-through">{formatPrice(template.original_price)}</span>}
